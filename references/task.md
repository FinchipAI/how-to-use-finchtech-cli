...
2. Create the draft and check its publication-review state before preparing publication through MCP. The four new Finch-reviewed Custom Task presets use `publication_custom`: website registration, product onboarding, product/feature trial, and form completion. They require Finch approval of the current revision before funding; standard goals and the legacy advanced `custom` goal retain their own rules. If publication requires review and no mounted review-submission tool is available, use the same Account's supported Browser review flow and wait for approval. Do not invent an MCP tool or fund an unapproved revision. After approval, call `identity_actor_get`, compare it with `finch status`, and hard stop if Account, wallet, or environment differs. Inspect the frozen pool preparation, show the exact transaction summary for approval, then run the returned `finch task publish-submit` action. It validates authority, broadcasts once, checkpoints the hash, and confirms atomically. Re-read approval after content changes; recover an existing funding operation through its returned recovery action instead of creating a second pool.
...
### Merchant review, settlement and regional admission

For `merchant_manual` and legacy `delayed_auto_7d` Tasks, merchants review during the seven days after the Task deadline, followed by two days of settlement grace. Neither mode automatically selects or awards remaining participants. Remaining funds become eligible for return to the merchant Vault at day nine, subject to the funded pool's immutable on-chain deadline and any dispute freeze. Report a return only after confirmed settlement evidence; elapsed time alone is not a receipt. Standard FCFS and weighted-lottery Tasks retain their respective automatic policies.

`review_later` marks an unfinished decision; untouched entries also remain unfinished. Exhausting awards or returning funds does not approve or reject these entries. Use the returned review window and remaining capacity when preparing an explicit award. Browser Excel import previews selected wallets and revalidates evidence; it is not approval or payment, and still requires the existing explicit confirmation and signing flow.
...
When X verification returns `manual_review` with `x_daily_credit_limit_exceeded` or `x_credit_budget_unavailable`, report the unmet verification requirement and returned recovery guidance. Required automatic checks must actually succeed before final submission; a budget error or bare manual-review status does not bypass admission. Web and MCP share the daily budget; respect returned retry limits. For requester-review goals, valid format evidence with its matching verification receipt can enter merchant review without becoming automatically verified or paid.
