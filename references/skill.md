...
3. Read delivery through MCP and run `finchtech skill download` to a new protected output file. Delivery packages use IPFS; pass the authoritative plan unchanged so the CLI verifies the immutable package hash before creating the file.
