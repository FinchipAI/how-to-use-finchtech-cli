...
## Publication diagnostics and recovery (CLI 0.3.3+)

Parse the JSON string in `error.state` for source diagnostics (`SKILL_SOURCE_SYMLINK_UNSUPPORTED`, `SKILL_SOURCE_UNREADABLE`) and final upload size (`SKILL_UPLOAD_SIZE_INVALID`, `actualBytes`, `maxBytes`). Paths are relative to the selected source. The CLI warns above 4,000,000 bytes and rejects above 4,500,000 bytes for the final multipart upload, including metadata and framing. The manifest allowance is separately 3 MiB. Source ZIP import accepts at most 10 MiB compressed and declared expanded content; this is not the final upload limit. Repacking may increase size. Correct the source rather than bypassing validation.
...
Intent errors identify `/api/skills/intents`, HTTP status and available safe correlation IDs. `responseIssue` distinguishes transport/body/JSON/schema failures; `schemaIssues` is bounded and contains no response values. Invalid success responses are rejected before signing or a pending-send checkpoint. Diagnostics do not repair server-side intent errors, manifest availability or Harness OAuth. Use the returned remote IDs for support; the publication fingerprint identifies a local journal, not a server trace.
...
CLI 0.3.3 writes a journal v6 pending stage before signing/submitting. A pending stage without a hash, or a legacy v4/v5 journal whose submission is uncertain, blocks automatic resubmission; contact support instead of deleting the journal. Known hashes are verified without rebroadcast. Do not remove local wallet/authorization state to bypass a pending publication. The CLI does not automatically retry intent requests or resubmit transactions.
